#!/usr/bin/env bash
# pipeline.sh: reduced-annotation PacBio long-read benchmark orchestration
set -euo pipefail
source ./config.sh

# Shared FASTQ data for both PacBio pipelines (this one and pacbio) — files
# are named with t{tissue}_s{sample}; reads don't change across
# reduced-annotation percentages. PacBio reads currently ship as FASTA, so
# match that too. Kept separate from data/long-read/FASTQ-ont/ — ONT and PacBio are
# distinct simulated datasets, not the same reads run through two aligners.
...
echo "============================================================"
echo " Reduced-annotation PacBio benchmarking: ${#fastq_files[@]} FASTQ file(s)"
echo "============================================================"
...
echo "============================================================"
echo " Reduced-annotation PacBio benchmarking pipeline complete!"
